...
if os.path.exists("large_forward.pkl"):
    ...
    cities_topological = {'fiedler': {}, 'tsne': {}, 'umap':{}, 'original':{}, 'random':{}}
...
for city in epsg:
    ...
    print('t-SNE - Mean:', statistics.mean(topological_tsne))
    print('t-SNE - Median:', statistics.median(topological_tsne))
    print('UMAP - Mean:', statistics.mean(topological_umap))
    print('UMAP - Median:', statistics.median(topological_umap))
    print('Original - Mean:', statistics.mean(topological_original))
    print('Original - Median:', statistics.median(topological_original))
    print('Random - Mean:', statistics.mean(topological_random))
    print('Random - Median:', statistics.median(topological_random))

    # Create a boxplot comparing the diagonal distances for each method
    plt.figure(figsize=(4, 3))
    plt.boxplot([topological_fiedler, topological_tsne, topological_umap, topological_original, topological_random],
                labels=['Fiedler', 't-SNE', 'UMAP', 'Original', 'Random'], showfliers=False)
    plt.xlabel("Index Ordering")
    plt.ylabel("Topological Approach")
    plt.savefig(city + '_topological.pdf', dpi=300, format='pdf', bbox_inches='tight')

    # Save the forward metrics for each city
    cities_topological['fiedler'][city] = topological_fiedler
    cities_topological['tsne'][city] = topological_tsne
    cities_topological['umap'][city] = topological_umap
    cities_topological['original'][city] = topological_original
    cities_topological['random'][city] = topological_random
